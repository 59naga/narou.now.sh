...
export default {
  ...
  created () {
    window.addEventListener('mousewheel', this.mousewheel, { passive: true })
  },
  destroyed () {
  ...
  methods: {
    ...
    mousewheel (event) {
      const visibleOverlay = document.querySelector('.v--modal-overlay')
      if (visibleOverlay) {
        return
      }
      let scrollVolume = event.wheelDelta / 120
      const em = document.querySelector('#reader p').clientWidth
      window.scrollTo(window.scrollX + scrollVolume * em, 0)
    }
  }
}
</script>
